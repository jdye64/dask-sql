...
pub struct DaskSqlOptimizer {
    optimizer: Optimizer,
    // The intermediate_plans that are generated by each optimization stage
    intermediate_plans: Vec<OptimizerOutput>,
...
impl DaskSqlOptimizer {
    ...
    > {
        ...
        // Keeps track of the last plan
        self.last_plan = Some(plan.clone());

        let closure = |optimized_plan: &LogicalPlan, optimization: &dyn OptimizerRule| {
            self.intermediate_plans.push(OptimizerOutput {
                optimizer_name: optimization.name().to_string(),
                input_plan: self.last_plan.clone().unwrap().display_indent().to_string(),
                output_plan: optimized_plan.display_indent().to_string(),
            });
            self.last_plan = Some(optimized_plan.clone());
        };

        // // Reset the last_plan to None
        // self.last_plan = None;

        let result = self.optimizer.optimize(&plan, &config, closure);
        Ok((Some(self.intermediate_plans.clone()), result))
    }
